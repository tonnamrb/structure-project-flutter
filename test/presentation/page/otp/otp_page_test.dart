...
import 'package:structure_project_flutter/app/controllers/otp_controller.dart';
...
import 'package:structure_project_flutter/presentation/page/otp/otp_page.dart';
...
void main() {
  ...
  testWidgets('OtpPage shows target value and confirm button', (tester) async {
    final mockAuth = MockAuthService();
    when(() => mockAuth.verifyOtp(any())).thenAnswer((_) async => true);
    when(() => mockAuth.sendOtp(channel: any(named: 'channel'), value: any(named: 'value')))
        .thenAnswer((_) async {});

    final controller = OtpController(authService: mockAuth);
    Get.put<AuthService>(mockAuth);
    Get.put<OtpController>(controller);
    controller.targetValue.value = '0123456789';
    ...
    expect(find.text('otp.confirm'.tr), findsOneWidget);
